...
describe('Multiple Users', () => {
  ...
  describe('Two Users in queue', () => {
    it('should have two users join, and when one is done, the second is next', async () => {
      ...
      const user1QueueLength = await await user1Page.innerText(
          queueLengthSelector,
      );
      ...
      async function assertPosition(page, position) {
        expect(await page.innerText('#position-in-queue')).toMatch(
            position.toString(),
        );
      }
